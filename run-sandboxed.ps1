...
# node_modules stays a named volume (install cache). Results bind-mount to a
# host directory so findings are readable on the host after a scan (override
# with $env:UOPFUZZ_RESULTS_DIR). The workspace itself is mounted read-only.
docker volume create uopfuzz-node-modules 2>$null | Out-Null
$ResultsDir = if ($env:UOPFUZZ_RESULTS_DIR) { $env:UOPFUZZ_RESULTS_DIR } else { Join-Path $ScriptDir 'results' }
