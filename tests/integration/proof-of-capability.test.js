...
describe('Proof: V8 coverage sees branches static analysis cannot distinguish', () => {
  ...
  test('different inputs produce measurably different V8 branch coverage', async () => {
    ...
    // Use the branching function defined at the top of this file. Static
    // analysis sees ALL branches. V8 coverage tells you WHICH ran.
    // It must live in a real file: a `new Function(...)` script has an empty
    // coverage URL, which takeCoverage() filters out, so its branches would
    // never be observable in the collected data.
    const fn = processInput;

    // Input 1: exercises admin+valid-token path
    fn({ type: 'admin', token: 'secret123' });
    ...
    // Input 3: exercises admin+wrong-token path
    fn({ type: 'admin', token: 'hunter2' });
    ...
    // Accumulate each block's offset AND taken-state per execution. V8 block
    // coverage reports a taken function as one count>0 range plus the UNTAKEN
    // branch arms as count:0 ranges, so different inputs yield different
    // offset:taken entries even though the function as a whole ran (count>0)
    // in every window.
    const coveredOffsets = (metrics) =>
      new Set(metrics.blocks.map(b => `${b.startOffset}-${b.endOffset}:${b.count > 0}`));

    const offsets1 = coveredOffsets(m1);
    const offsets2 = coveredOffsets(m2);

    // Proof: at least one offset:taken entry is unique to each input
    // (input 1 leaves the guest arm as a count:0 gap that input 2 takes,
    // and vice versa for the admin arms)
