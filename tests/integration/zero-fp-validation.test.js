...
describe('harness never self-triggers a polluted engine-invoked property', () => {
  ...
  // The gap the benign fixture missed (real-world: date-fns@4.1.0 set()): a
  // library that coerces its argument to a primitive — set() does `new Date(...)`
  // — makes the JS ENGINE invoke a polluted toString/valueOf. That is engine
  // coercion, not the target routing attacker data to a code sink, and a PP
  // attacker can only inject data via JSON, never a live function. It must never
  // confirm code execution.
  test('reproduction does not confirm RCE when the TARGET coerces its argument', async () => {
    for (const property of ['toString', 'valueOf', 'toJSON']) {
      const r = await reproduceRce(FIX('coercion-trap'), 'set', { property, gates: [], minimalArgs: [{}] });
      assert.equal(r.verified, false,
        `${property}: engine coercion inside the target must not confirm code execution`);
    }
  });

  // The SAME unrealizability applies beyond the engine-coercion names: any time
  // the target EXPLICITLY method-dispatches a polluted property (`obj.split()`,
  // `obj.replace()`, `for..of` → @@iterator), a live function on the prototype
  // is invoked and fires the canary — yet a prototype-pollution attacker can
  // only inject DATA, never a callable. These are the real-world false
  // positives the engine-invoked-only guard missed: 6to5 (split), marked
  // (replace), immutable (@@iterator). The function_call proof must not confirm
  // for ANY property; only the string proofs (eval_string / sink_reach) can.
  test('reproduction does not confirm RCE from a live function on an explicitly method-dispatched property', async () => {
    for (const property of ['split', 'replace']) {
      const r = await reproduceRce(FIX('method-dispatch-fp'), 'tokenize', { property, gates: [], minimalArgs: [{}] });
      assert.equal(r.verified, false,
        `${property}: a live function reached only because the target calls obj.${property}() is not PP-realizable and must not confirm`);
    }
  });
});
