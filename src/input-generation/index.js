...
const PAYLOADS = [
  ...
  // TEST-NET-1 (RFC 5737): unreachable, safe even with --allow-network.
  { type: 'ssrf_url', value: 'http://192.0.2.1/uopfuzz-canary' },
 ...
 *
 * - Power schedule: seeds that discover new coverage get exponentially more
 *   mutations (coverage-based greybox fuzzing, Böhme et al., CCS 2016).
 * - Mutation strategy selection uses Thompson sampling over per-strategy
 *   success (see selectStrategiesByThompson).
 * - Prototype-pollution mutation patterns follow the "Silent Spring" gadget
 *   taxonomy (Shcherbakov et al., USENIX Security 2023).
 */
export class InputGeneration {
   ...
   */
  applyVerticalChaining(input, _config) {
    if (input.type !== 'object') return null;
   ...
   * Libraries", ICSE 2020
   */
  applyTypeCoercion(input, _config) {
    const coercions = [
   ...
   * hijack async control flow: a polluted `then` on Object.prototype can be
   * adopted by the engine's thenable resolution when a plain object is awaited
   * or returned from a promise.
   */
  applyAsyncPollution(input, _config) {
    if (input.type !== 'object') return null;
