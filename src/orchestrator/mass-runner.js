...
import { buildSandboxChildEnv } from '../utils/sandbox.js';
...
export function runOrchestratorIsolated(options, { timeoutMs = DEFAULT_PER_TARGET_TIMEOUT_MS, workerPath = SUBPROCESS_PATH, silent = false } = {}) {
  return new Promise((resolve) => {
    const child = fork(workerPath, [], {
      ...
      // SECURITY: target module-eval and discovery code run in this child —
      // scrub secret-bearing env vars, same as the sandbox grandchildren.
      env: buildSandboxChildEnv(false),
    });
