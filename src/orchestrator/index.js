...
// Real timers, captured from node:timers rather than the global. The
// instrumentation temporarily replaces global.setTimeout/setInterval with a
// sink-logging stub (src/instrumentation/index.js setupSinkTracing) that returns
// a sentinel string instead of a Timeout handle. The orchestrator's own
// control-flow timers must never go through that stub, or clearTimeout throws on
// the sentinel string. node:timers exports are separate bindings, unaffected by
// `global.setTimeout = …`, so these stay real even while a sink is instrumented.
...
// Cap unbounded potential-chain growth in the fuzzing loop; final ranking only
// uses the top candidates anyway (dedup happens at report time).
const MAX_POTENTIAL_CHAINS = 10000;

/**
 * Fuzzing orchestrator.
 *
 * Coordinates the pipeline:
 *  - a coverage-guided feedback loop (edge coverage steers input-generation
 *    power schedules; Böhme et al., CCS 2016),
 *  - convergence detection from coverage saturation, and
 *  - reporting of confirmed findings (each independently reproduced) plus
 *    unconfirmed leads.
 */
export class Orchestrator {
  ...
  async loadConfiguration() {
    ...
    try {
      ...
      // Fail fast with a clear error instead of per-trace TypeErrors later.
      new TargetIntegration(this.options).validateConfig(this.config);
  ...
  async executeFuzzingWorkflow() {
    ...
    // --parallel N is pool-backed: the differential probe loop (Phase B of the
    // sequential workflow) dispatches up to N descriptor probes concurrently
    // through the sandbox worker pool (Instrumentation sizes the pool from
    // parallelWorkers). The zero-FP gate is untouched — proveAndRecord is
    // serialized via _serialConfirm, so parallel probes cannot double-record,
    // and the confirmed result set matches the sequential run.
    if (parallelWorkers > 1) {
      logger.info(`Using pool-backed parallel probes (${parallelWorkers} sandbox workers)`);
    } else {
      ...
      try {
        ...
        const traces = await Promise.race([
          ...
          new Promise((_, reject) => {
            // realSetTimeout, not the global: executeWithTracing (racing in this
            // same Promise.race) may have already swapped global.setTimeout for
            // the sink stub, which returns a sentinel string rather than a timer.
            timeoutTimer = realSetTimeout(() => reject(new Error('Iteration timeout')), timeout);
          })
        ]);
        realClearTimeout(timeoutTimer);
        ...
        if (this.results.potentialChains.length < MAX_POTENTIAL_CHAINS) {
          this.results.potentialChains.push(...chains.slice(0, MAX_POTENTIAL_CHAINS - this.results.potentialChains.length));
        } else {
          logger.debug(`potentialChains cap (${MAX_POTENTIAL_CHAINS}) reached — later candidates dropped`);
        }
  ...
  async proveAndRecord(diffResult, testInput, descriptor, extra = {}) {
    ...
    if (proof?.verified) {
      ...
      // Distinguish PROVEN EXECUTION (a canary fired — eval_string /
      // constructor_chain) from mere SINK REACHABILITY (the polluted value
      // reached a sink argument as a string — sink_reach). The latter is a real,
      // useful finding (it's how command injection is confirmed, since spawning
      // is unsafe to actually run), but it is NOT proven code execution and is
      // sanitization-blind, so it must not be reported as "canary fired" RCE.
      const isSinkReach = proofType === 'rce' && proof.payloadType === 'sink_reach';
      chain.proof = {
        type: proofType === 'pp'
          ? 'prototype-pollution'
          : isSinkReach ? 'sink-reachability' : 'code-execution',
        verified: true,
        runs: proof.runs,
  ...
  async executeDifferentialPhase(inputs, iteration) {
    ...
    // Dispatch descriptor probes. --parallel N > 1 runs up to N descriptors
    // concurrently (batched Promise.all, batch size = N); each descriptor keeps
    // its exact sequential EP/mode walk, and proveAndRecord stays serialized
    // through _serialConfirm, so the confirmed finding set matches the
    // sequential run (which representative property records a shared bug can
    // differ under concurrency — the set does not). With parallel = 1 this is
    // the plain sequential loop.
    const parallel = Math.max(1, this.options.parallelWorkers || 1);
    const probeCtx = { testInputs, urlSinkEPs, getBaseName, urlSinkProperties };
    for (let i = 0; i < descriptors.length; i += parallel) {
      const batch = [];
      for (const descriptor of descriptors.slice(i, i + parallel)) {
        ...
        // .catch: one rejecting descriptor must not cancel its batchmates'
        // confirms (Promise.all fail-fast would abort the rest of the batch).
        batch.push(this._probeDescriptor(descriptor, probeCtx).catch((err) => {
          logger.debug(`Parallel probe for ${descriptor.property} failed: ${err.message}`);
        }));
      }
    ...
    // Runs after all descriptor batches complete (the confirm chain has drained),
    // so it stays sequential exactly as before.
            ...
            try {
              ...
              if (md && (md.isConfirmedGadget || (md.isCandidate && md.reproducible))) {
                ...
                if (!this._confirmedSignatures.has(sig)) {
                  ...
                  const proven = await this._serialConfirm(multiResult, coTestInput, payloadDesc, {
                    gates: [gateProp],
  ...
  /**
   * Serialize proveAndRecord calls. Concurrent descriptor probes (--parallel N)
   * would otherwise interleave the async reproduction step: the _attemptedRepro
   * check-then-fork-then-add spans the reproduction await, so two probes for the
   * same underlying bug could both pay the 2-fork reproduction before either
   * records. (The report-dedup check-and-add itself is synchronous and already
   * atomic; this queue exists to dedupe the expensive reproduction forks.)
   */
  _serialConfirm(diffResult, testInput, descriptor, extra = {}) {
    if (!this._confirmChain) this._confirmChain = Promise.resolve();
    const run = this._confirmChain.then(() => this.proveAndRecord(diffResult, testInput, descriptor, extra));
    // Keep the chain alive even if a confirm rejects (proveAndRecord already
    // swallows reproduction errors; this is belt-and-braces).
    this._confirmChain = run.catch(() => {});
    return run;
  }

  /**
   * Probe one pollution descriptor against the ranked test inputs — all four
   * differential modes, sequential per descriptor with the same early-break
   * rules as the original loop. Shared by the sequential path and the
   * pool-backed parallel dispatch; under --parallel N, N descriptors run this
   * concurrently while confirmation stays serialized via _serialConfirm.
   */
  async _probeDescriptor(descriptor, { testInputs, urlSinkEPs, getBaseName, urlSinkProperties }) {
      const sig = `${descriptor.property}:${descriptor.payloadType}`;
        ...
        try {
          ...
          if (diffResult) {
            ...
            if (d && (d.isConfirmedGadget || (d.isCandidate && d.reproducible))) {
              confirmed = await this._serialConfirm(diffResult, testInput, descriptor);
          ...
          if (mergeResult?.diff) {
            confirmed = await this._serialConfirm(mergeResult, testInput, descriptor);
            if (confirmed) { this._confirmedSignatures.add(sig); break; }
          }
          ...
          if (urlResult?.diff) {
            confirmed = await this._serialConfirm(urlResult, testInput, descriptor);
            if (confirmed) { this._confirmedSignatures.add(sig); break; }
          }
          ...
          if (!confirmed && !epIsUrlSink) {
            ...
            if (fd && (fd.isConfirmedGadget || (fd.isCandidate && fd.reproducible))) {
              ...
              confirmed = await this._serialConfirm(forcedResult, testInput, descriptor, {
                gates,
  ...
  /**
   * Generate a plain-text analysis report: confirmed (reproduced) findings,
   * unconfirmed leads, coverage/strategy diagnostics, and per-finding severity.
   */
  generateReport() {
    ...
    if (this.results.potentialChains.length > 0) {
      ...
      this.results.potentialChains.slice(0, 20).forEach((chain, index) => {
        ...
        report += `   Severity: ${chain.riskLevel || 'N/A'}/10\n`;
        report += `   Rank score: ${((chain.confidence || 0) * 100).toFixed(1)}%\n`;
    ...
    report += `\nMethodology\n`;
    report += `===========\n`;
    report += `- Differential oracle: clean vs. polluted execution comparison\n`;
    report += `- Reproduction gate: every reported finding is re-proven in fresh child processes\n`;
    report += `- UOP discovery: Proxy-based detection of undefined property reads\n`;
    report += `- Coverage guidance: AFL-style edge coverage bitmap, plus V8 precise coverage\n`;
    report += `- Taint tracking: ES6 Proxy deep property interception\n`;
    report += `- Strategy selection: Thompson sampling over per-strategy success\n`;
    report += `- Severity: coarse heuristic by sink class (not a computed CVSS score)\n`;
    report += `- Gadget taxonomy follows "Silent Spring" (Shcherbakov et al., USENIX 2023)\n`;
