// Real timer, captured from node:timers rather than the global: sink tracing
// replaces global.setTimeout with a logging stub, and the microtask drains
// below must actually yield to the event loop.
  ...
  try {
    try {
      Object.defineProperty(Object.prototype, prop, {
        ...
        configurable: true,
        // Non-enumerable on the DISCOVERY side: an enumerable accessor on
        // Object.prototype destabilizes the many `for..in` loops inside jsdom and
        // other targets during the probe, and discovery only needs to PROPOSE a
        // candidate (the getter already fires when a `for (k in opts)` gadget
        // READS opts[k]). Reproduction — the actual confirmation gate — installs
        // the payload ENUMERABLE (see installProp in repro-worker.js), so a
        // genuine for..in gadget is still faithfully reproduced there.
        enumerable: false,
      });
    } catch {
...
function diffResults(cleanResult, pollutedResult, pollutionDescriptor) {
  ...
  // Check if polluted payload appears in output (data-flow confirmation). Only
  // meaningful for a DISTINCTIVE string/number payload: a boolean `true`/`false`
  // (the common gate value) stringifies to a token that matches almost any
  // serialized output ("b:true" contains "true"), which used to flip this on
  // spuriously and inflate the Injection-tier CVSS. Restrict to string/number
  // payloads of a few chars, and match under the same reversible transforms real
  // gadgets apply (argContainsTransformedValue).
  const rawVal = pollutionDescriptor.value;
  let payloadInOutput = false;
  if ((typeof rawVal === 'string' || typeof rawVal === 'number')) {
    const payloadStr = String(rawVal);
    if (payloadStr.length >= 3) {
      payloadInOutput = !!pollutedOutput && argContainsTransformedValue(pollutedOutput, payloadStr) &&
                        !(cleanOutput && argContainsTransformedValue(cleanOutput, payloadStr));
    }
  }
...
export async function executeMergePPTest(fn, baseArgs, prop, val, timeoutMs = 5000) {
  ...
  const payloads = [
    { payload: JSON.parse(`{"__proto__":{${JSON.stringify(prop)}:${JSON.stringify(val)}}}`), payloadKind: '__proto__' },
    { payload: { constructor: { prototype: { [prop]: val } } }, payloadKind: 'constructor.prototype' },
  ];
  ...
  for (const { payload, payloadKind } of payloads) {
    for (const variant of argVariants) {
    const snapshot = snapshotPrototype();

    const args = variant.map((arg) => {
  ...
  const urlPayloads = [
    {
      ...
      object: JSON.parse(`{"__proto__":{${JSON.stringify(prop)}:${valStr}}}`),
    ...
    },
    {
      ...
      object: JSON.parse(`{${JSON.stringify(prop)}:{"__proto__":{${JSON.stringify(prop)}:${valStr}}}}`),
      payloadType: 'url_nested___proto__',
    },
    {
      url: `http://localhost/?a[__proto__][${encodeURIComponent(prop)}]=${encodeURIComponent(val)}`,
      object: JSON.parse(`{"a":{"__proto__":{${JSON.stringify(prop)}:${valStr}}}}`),
