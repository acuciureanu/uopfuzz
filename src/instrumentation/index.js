...
// Cap on traced property accesses per trace — hot loops calling the
// hasOwnProperty/getOwnPropertyDescriptor hooks would otherwise grow
// trace.propertyAccesses without bound.
const MAX_PROPERTY_ACCESSES = 100000;
...
export class Instrumentation {
  constructor(options) {
    ...
    // Normalize the pool concurrency from the orchestrator's --parallel flag
    // (parallelWorkers) so the sandbox pool is sized correctly on first use —
    // and the setting survives pool teardown at run end.
    if (this.options && this.options.maxConcurrency == null) {
      this.options.maxConcurrency = Math.max(1, this.options.parallelWorkers || 1);
    }
    this.traces = [];
  ...
  _getPool() {
    if (!this.sandboxPool) {
      // --parallel N (orchestrator option parallelWorkers) sizes the pool: up to
      // N persistent sandbox workers per package key, so N discovery probes can
      // execute concurrently. Reproduction never touches this pool.
      // (The constructor already normalized options.maxConcurrency.)
  ...
  async executeInputWithTaintTracking(input, config, trace) {
    ...
    if (taintLog.length > 0) {
      ...
      for (const event of taintLog) {
        if (event.type === 'get' && trace.propertyAccesses.length < MAX_PROPERTY_ACCESSES) {
  ...
  async executeDifferentialTracing(input, config, pollutionDescriptor) {
    ...
    if (canSandbox) {
      return this._sandboxedDifferential(config.package, input, pollutionDescriptor, timeoutMs, config.browserEnv === true);
    }
   ...
   */
  async _sandboxedDifferential(packageName, input, descriptor, timeoutMs, browserEnv = false) {
    try {
      ...
      const result = await this._getPool().run(packageName, input.entryPoint, args, {
        ...
        mode: 'differential',
        // config.browserEnv is set at setup time for targets loaded via jsdom
        // (including fixtures/DOM-detection fallbacks); the name check covers
        // the well-known browser-only packages.
        browserEnv: browserEnv || isBrowserOnly(packageBaseName(packageName)),
        collectCoverage: this.options.collectCoverage !== false,
      });

      if (result.error && !result.outputChanged && !result.prototypePolluted) {
        logger.debug(`Sandboxed differential failed: ${result.error}`);
        this._consumeSandboxCoverage(result);
        return null;
      }
  ...
  _reconcileSandboxDiff(descriptor, result, extraDetails = {}, displayProperty = null) {
    // Phase-B → Phase-A coverage feedback, BEFORE the early returns below — an
    // unconfirmed probe's execution work still counts.
    this._consumeSandboxCoverage(result);
    ...
    // Distinctive string/number payloads only, matched under the reversible
    // transforms real gadgets apply. A boolean gate value ("true"/"false") would
    // otherwise match nearly any serialized output and spuriously raise the
    // Injection tier — see the same guard in differential.js#diffResults.
    let payloadReachedOutput = false;
    {
      const rawVal = descriptor.value;
      if ((typeof rawVal === 'string' || typeof rawVal === 'number')) {
        const payloadStr = String(rawVal);
        const pol = result.polluted?.output;
        const cln = result.clean?.output;
        if (payloadStr.length >= 3 && typeof pol === 'string') {
          payloadReachedOutput = argContainsTransformedValue(pol, payloadStr) &&
            !(typeof cln === 'string' && argContainsTransformedValue(cln, payloadStr));
        }
      }
    }
  ...
  async executeForcedBranchDifferentialTracing(input, config, pollutionDescriptor) {
    ...
    const canSandbox = this.options.sandbox && config.package;
    if (canSandbox) {
      return this._sandboxedForcedBranch(config.package, input, pollutionDescriptor, DIFF_CALL_TIMEOUT_MS, config.browserEnv === true);
    }
   ...
   */
  async _sandboxedForcedBranch(packageName, input, descriptor, timeoutMs, browserEnv = false) {
    try {
      const safeDescriptor = {
        property: descriptor.property,
  ...
  async executeMultiPropertyDifferentialTracing(input, config, descriptors) {
    ...
    const canSandbox = this.options.sandbox && config.package;
    if (canSandbox) {
      return this._sandboxedMultiProperty(config.package, input, descriptors, DIFF_CALL_TIMEOUT_MS, config.browserEnv === true);
    }
   ...
   */
  async _sandboxedMultiProperty(packageName, input, descriptors, timeoutMs, browserEnv = false) {
    try {
  ...
  async executeMergePPDifferential(input, config, descriptor) {
    ...
    if (canSandbox) {
      ...
      return this._sandboxedMergePP(config.package, input.entryPoint, descriptor, timeoutMs, config.browserEnv === true);
    }
   ...
   */
  async _sandboxedMergePP(packageName, entryPoint, descriptor, timeoutMs, browserEnv = false) {
    try {
      const safeDescriptor = {
        property: descriptor.property,
        value: typeof descriptor.value === 'function' ? '__UOPFUZZ_MARKER_7f3a__' : descriptor.value,
      ...
      // Merge-PP doesn't go through _reconcileSandboxDiff — feed coverage here.
      this._consumeSandboxCoverage(result);
  ...
  async executeURLGadgetDifferential(input, config, descriptor) {
    ...
    if (canSandbox) {
      const urlInput = {
        ...input,
        value: JSON.parse(`{"__proto__":{${JSON.stringify(descriptor.property)}:${JSON.stringify(descriptor.value)}}}`),
      };
      return this._sandboxedDifferential(config.package, urlInput, descriptor, isUrlSink ? DIFF_URL_SINK_TIMEOUT_MS : DIFF_CALL_TIMEOUT_MS, config.browserEnv === true);
    }
    ...
    if (canSandbox) {
      try {
        ...
        const result = await this._getPool().run(config.package, input.entryPoint, args, {
          ...
          blockNetwork: this.options.blockNetwork !== false,
          mode: 'discover_uop',
          browserEnv: config.browserEnv === true || isBrowserOnly(packageBaseName(config.package)),
        });
  ...
  /**
   * Feed a V8 ScriptCoverage snapshot collected by a SANDBOX WORKER into the
   * AFL-style bitmap, reusing the exact conversion of the in-process Layer 1b
   * path (extractMetrics → feedV8CoverageIntoBitmap) so edge identity matches.
   * Called from _reconcileSandboxDiff for every probe that carried coverage.
   *
   * @param {Array} rawCoverage - V8 ScriptCoverage[] from the worker reply
   */
  _feedSandboxV8Coverage(rawCoverage) {
    try {
      const metrics = this.v8Collector.extractMetrics(rawCoverage);
      this.feedV8CoverageIntoBitmap(metrics, null);
    } catch (error) {
      logger.debug(`Sandbox V8 coverage feed failed: ${error.message}`);
    }
  }

  /**
   * Consume the worker's v8Coverage field from a sandbox probe result and feed
   * it into the bitmap. The field is nulled on the result so a result passing
   * through a second path can never be fed twice, and the returned diff never
   * carries the raw snapshot. Call this on EVERY sandbox result — including
   * error results that return before _reconcileSandboxDiff — so every probe's
   * execution work feeds back (Phase B → Phase A).
   */
  _consumeSandboxCoverage(result) {
    const v8Coverage = result?.v8Coverage;
    if (result) result.v8Coverage = null;
    if (Array.isArray(v8Coverage) && v8Coverage.length > 0) {
      this._feedSandboxV8Coverage(v8Coverage);
    }
  }
  ...
  setupPropertyTracing(trace) {
    ...
    Object.getOwnPropertyDescriptor = function(obj, prop) {
      if (obj && typeof obj === 'object') {
        if (trace.propertyAccesses.length < MAX_PROPERTY_ACCESSES) {
          trace.propertyAccesses.push({
  ...
  setupSinkTracing(trace, sinks) {
    ...
    try {
      if (childProcessModule && childProcessModule.exec) {
        ...
        // Tracked so cleanupTracing() restores it — otherwise every later
        // require('child_process').exec in this process gets the simulated stub.
        this.instrumentedFunctions.add({
          name: '__childProcessExec__',
          original: originalExec,
          restore: () => { childProcessModule.exec = originalExec; },
        });
   ...
   */
  buildCallArgs(step, input, _config) {
    const args = [];
  ...
  async safeExecute(fn, args) {
    let timer;
    // Silence the whole console family while the target runs — frameworks and
    // chatty libraries (e.g. TypeScript's API) spam stdout/stderr on every call,
    // which would drown the fuzzer's own logger output. winston writes to the
    // stream directly, not through console.*, so the fuzzer's logs stay visible.
    console.error = _noopFn;
    console.warn = _noopFn;
    console.log = _noopFn;
    console.info = _noopFn;
    console.debug = _noopFn;
    try {
      const timeout = new Promise((_, reject) => {
        // realSetTimeout, not the global: safeExecute runs while sink tracing has
        // replaced global.setTimeout with a stub that returns a sentinel string,
        // so a global timer here would make clearTimeout throw on that string.
  ...
  async simulateExecution(input, config, trace) {
    ...
    // realSetTimeout: this drain runs inside the sink-tracing window, where the
    // global setTimeout may be the sentinel-returning stub (which never fires the
    // callback, hanging the await).
  ...
  cleanupTracing() {
    ...
    for (const instrumented of this.instrumentedFunctions) {
      try {
        if (instrumented.restore) instrumented.restore();
        else global[instrumented.name] = instrumented.original;
      } catch (error) {
