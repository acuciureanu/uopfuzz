...
export class V8CoverageCollector {
  ...
  async start() {
    ...
    try {
      ...
      // Don't leak a connected session when setup throws.
  ...
  async takeCoverage() {
    ...
    // Exclude node internals AND the fuzzer's own code (it runs from src/, so
    // the old node_modules/uopfuzz check never matched). Counting our own
    // instrumentation would inflate the AFL bitmap with fuzzer edges.
    const selfSrc = path.resolve(__dirname, '..') + path.sep;
    return result.filter(script =>
      !script.url.startsWith('node:') &&
      script.url !== '' &&
      !script.url.includes('node_modules/uopfuzz') &&
      !(script.url.startsWith('file://') && fileURLToPath(script.url).startsWith(selfSrc))
    );
  }
