...
export function executeInSandbox(packageName, entryPoint, args, options = {}) {
  const {
    ...
    // ScriptCoverage snapshot (`v8Coverage`) to the reply. Default off — the
    // per-probe session has a real cost.
  ...
  return new Promise((resolve, _reject) => {
    ...
    // Per-fork reply token: only a message echoing it may settle this promise.
    // Target code loaded in the child cannot forge a `verified:true` reply — it
    // can't reach the (sealed) process.send, and never sees this token. Without
    // this, a hostile package could manufacture a confirmed finding by sending a
    // fabricated IPC message. See sealProcessSend() in worker-hardening.js.
    ...
    child.on('message', (msg) => {
      if (settled) return;
      // Ignore any message that doesn't echo our token: a forged reply from
      // target code (untagged, or a wrong guess) never settles the promise;
      // the parent watchdog above still resolves if no genuine reply arrives.
      if (!msg || msg.__replyToken !== replyToken) return;
      settled = true;
      clearTimeout(timer);
      child.kill('SIGTERM');
      const result = { ...msg };
      delete result.__replyToken;
      resolve(result);
    });
