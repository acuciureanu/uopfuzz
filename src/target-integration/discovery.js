...
function looksLikeMerge(fn) {
  ...
  try {
    ...
    if (mergedDeeply(target, defuse(withSuppressedConsole(() => fn(target, makeSource()))))) return true;
  } catch { /* not this convention */ }

  try {
    const target = { nest: { existing: 1 } };
    if (mergedDeeply(target, defuse(withSuppressedConsole(() => fn(true, target, makeSource()))))) return true;
...
function looksLikeFactory(fn) {
  for (const args of FACTORY_PROBE_ARGS) {
    try {
      const value = defuse(withSuppressedConsole(() => fn(...args)));
 ...
 */
async function probeFunction(fn, _name) {
...
// Cached console references — avoid saving/restoring per call. We silence the
// whole console family (not just error/warn): probing thousands of exports on a
// chatty package (e.g. TypeScript's compiler API prints via console.log) would
// otherwise flood the terminal with target output. The fuzzer's own status lines
// come from the winston logger, which writes to the stream directly rather than
// through console.*, so they stay visible.
const _origConsole = {
  error: console.error,
  warn: console.warn,
  log: console.log,
